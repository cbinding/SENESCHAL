--old
/*
SELECT DISTINCT
	tu2.list_uid AS scheme_id,
	tu2.the_te_uid AS topconcept_id
FROM	rc_thesaurus_uses tu
INNER JOIN
	rc_thesaurus_uses tu2
ON	tu.top_term_u_uid = tu2.the_t_u_uid
INNER JOIN
	rc_thesaurus_terms tt
ON	tu.the_te_uid = tt.the_te_uid
WHERE 	tt.status = 'Y' 
AND	tu2.the_t_u_uid = tu2.top_term_u_uid
AND	tu.list_uid IN ('1');
*/

--new
SELECT DISTINCT
	tt.list_uid AS scheme_id,
	tt.the_te_uid AS topconcept_id,
	tt.term AS preflabel
FROM	rc_thesaurus_uses tu
INNER JOIN
	rc_thesaurus_terms tt
ON	tu.list_uid = tt.list_uid
AND	tu.the_te_uid = tt.the_te_uid
WHERE 	tu.the_t_u_uid = tu.top_term_u_uid
AND	tt.status = 'Y' 
AND	tu.list_uid IN ('1')
ORDER BY preflabel;